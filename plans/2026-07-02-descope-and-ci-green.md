...
- [ ] **Step 3b (accumulated from Tasks 1-6):** Purge references to ALL deleted targets from `project.yml`, regenerate `Spooktacular.xcodeproj` (xcodegen) or hand-edit `project.pbxproj`, and clean `fastlane/Fastfile` lanes referencing deleted bundle ids (e.g. `com.spooktacular.app.NetworkFilter` at Fastfile ~13,147,792 and any MDM/agent equivalents). Verify `xcodebuild -scheme Spooktacular -destination 'platform=macOS' build` locally.
