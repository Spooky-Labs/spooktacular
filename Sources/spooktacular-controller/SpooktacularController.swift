...
/// | `SPOOKTACULAR_TENANCY_MODE` | `single-tenant` | `single-tenant` or `multi-tenant` |
/// | `SPOOKTACULAR_TENANT_CONFIG` | (none) | Path to JSON tenant-pool mapping file |
/// | `SPOOKTACULAR_AUDIT_MERKLE` | (none) | Set to `1` to enable Merkle audit sink |
/// | `SPOOKTACULAR_AUDIT_SIGNING_KEY` | (none) | Path to Ed25519 signing key for Merkle audit |
...
struct SpooktacularController {
    ...
    static func main() async {
        ...
        switch env["SPOOKTACULAR_TENANCY_MODE"] ?? "single-tenant" {
        case "multi-tenant":
        ...
        // Load tenant-pool mapping from SPOOKTACULAR_TENANT_CONFIG file or environment
        let tenantConfig: TenantConfig?
        let tenantPools: [TenantID: Set<HostPoolID>]
        if let configPath = env["SPOOKTACULAR_TENANT_CONFIG"],
           ...
           let config = try? JSONDecoder().decode(TenantConfig.self, from: configData) {
            ...
            if tenancyMode == .multiTenant {
                logger.warning("Multi-tenant mode but no SPOOKTACULAR_TENANT_CONFIG — all tenants will use default pool")
            }
        }
        ...
        do {
            roleStore = try JSONRoleStore(configPath: env["SPOOKTACULAR_RBAC_CONFIG"])
            logger.notice("RBAC enabled: \(env["SPOOKTACULAR_RBAC_CONFIG"] ?? "built-in roles")")
        } catch {
        ...
        if let auditPath = env["SPOOKTACULAR_AUDIT_FILE"] {
            do {
            ...
            logger.notice("Audit sink: os.Logger (use SPOOKTACULAR_AUDIT_FILE for SIEM export)")
        }

        // Immutable append-only store (SPOOKTACULAR_AUDIT_IMMUTABLE_PATH)
        if let immutablePath = env["SPOOKTACULAR_AUDIT_IMMUTABLE_PATH"] {
            do {
        ...
        // Merkle tree tamper-evidence (auto in multi-tenant, or SPOOKTACULAR_AUDIT_MERKLE=1).
        //
        ...
        if tenancyMode == .multiTenant || env["SPOOKTACULAR_AUDIT_MERKLE"] == "1" {
        ...
        // IdP registry (SPOOKTACULAR_IDP_CONFIG)
        ...
        // Required in production. Use SPOOKTACULAR_INSECURE_CONTROLLER=1 to bypass
        // (development only — logs a prominent warning).
        let insecure = env["SPOOKTACULAR_INSECURE_CONTROLLER"] == "1"
        ...
        let certPath = env["SPOOKTACULAR_TLS_CERT_PATH"] ?? env["TLS_CERT_PATH"]
        let keyPath  = env["SPOOKTACULAR_TLS_KEY_PATH"]  ?? env["TLS_KEY_PATH"]
        ...
        if let certPath, let keyPath, let caPath {
            ...
            logger.fault("mTLS is required. Set SPOOKTACULAR_TLS_CERT_PATH, SPOOKTACULAR_TLS_KEY_PATH, and SPOOKTACULAR_TLS_CA_PATH, or set SPOOKTACULAR_INSECURE_CONTROLLER=1 for development.")
            return
        }
        ...
        // sets both SPOOKTACULAR_SCHEDULER_POLICY (path to a JSON
        // policy file) and SPOOKTACULAR_FLEET_CAPACITY (total VM
        ...
        let leaderElection = LeaderElection(client: client, leaseName: "spooktacular-controller")
    ...
    ///
    /// `SPOOKTACULAR_SCHEDULER_POLICY` points at a JSON file shaped like:
    ///
    /// ```json
    /// [
    ...
    /// ```
    ///
    /// `SPOOKTACULAR_FLEET_CAPACITY` is the integer total of VM slots
    ...
    static func loadFairScheduler(
        ...
        guard let policyPath = env["SPOOKTACULAR_SCHEDULER_POLICY"],
              !policyPath.isEmpty,
              let capacityRaw = env["SPOOKTACULAR_FLEET_CAPACITY"],
        ...
        guard let data = try? Data(contentsOf: URL(filePath: policyPath)),
            ...
            logger.fault(
                "SPOOKTACULAR_SCHEDULER_POLICY at '\(policyPath, privacy: .public)' missing or malformed — fair-share disabled"
            )
            return (nil, 0)
        }
...
///
/// Load from `SPOOKTACULAR_TENANT_CONFIG` file path. Example:
/// ```json
/// {
///   "tenantPools": {
///     "team-a": ["pool-1"],
