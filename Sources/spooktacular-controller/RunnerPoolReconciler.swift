...
actor RunnerPoolReconciler {
    ...
    /// that don't configure `SPOOKTACULAR_SCHEDULER_POLICY`.
    ...
    private func createRunner(name: String, sourceVM: String, pool: RunnerPool) async {
        ...
        guard isolation.canSchedule(tenant: tenantID, onto: hostPoolID) else {
            ...
            let context = AuthorizationContext(
                actorIdentity: "spooktacular-controller",
                tenant: tenantID,
                scope: .runner,
                resource: name,
    ...
    private func sourceVMExists(name: String, on endpoint: NodeEndpoint) async -> Bool {
        ...
        if let token = ProcessInfo.processInfo.environment["SPOOKTACULAR_API_TOKEN"], !token.isEmpty {
