...
public struct RegistrationTokenResponse: Codable, Sendable {
    ...
    /// Parses the ISO-8601 timestamps GitHub returns on
    /// runner / workflow objects.  Handles both the
    /// fractional-seconds variant (new endpoints) and the
    /// plain-seconds variant (older endpoints) via one
    /// [`Date.ISO8601FormatStyle`](https://developer.apple.com/documentation/foundation/date/iso8601formatstyle)
    /// call — empirically verified on macOS 26 to accept
    /// both input shapes.
