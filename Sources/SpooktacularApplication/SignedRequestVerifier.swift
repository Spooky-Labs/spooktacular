...
///
/// ```
/// X-Spooktacular-Timestamp: 2026-04-17T18:30:00Z      (ISO-8601, seconds)
/// X-Spooktacular-Nonce:     <uuid>                    (128-bit random)
/// X-Spooktacular-Signature: <base64(64-byte r‖s)>     (P-256 ECDSA raw)
/// ```
///
...
public final class SignedRequestVerifier: @unchecked Sendable {
    ...
    public func verifyFleetWide(
        ...
        guard
            let nonce = headers["x-spooktacular-nonce"]?.trimmingCharacters(in: .whitespaces),
            !nonce.isEmpty
        else {
    ...
    public func verify(
        ...
        guard
            let timestamp = headers["x-spooktacular-timestamp"]?.trimmingCharacters(in: .whitespaces),
            let nonce = headers["x-spooktacular-nonce"]?.trimmingCharacters(in: .whitespaces),
            let signatureB64 = headers["x-spooktacular-signature"]?.trimmingCharacters(in: .whitespaces),
    ...
    /// our signers emit, plus fractional-second variants.
    ///
    /// Uses [`Date.ISO8601FormatStyle`](https://developer.apple.com/documentation/foundation/date/iso8601formatstyle) —
    /// Apple's recommended replacement for `ISO8601DateFormatter`
    /// (see Apple's own tip on the
    /// [ISO8601DateFormatter docs page](https://developer.apple.com/documentation/foundation/iso8601dateformatter)).
    /// The modern API is `Sendable` by construction, so no
    /// `nonisolated(unsafe)` wrapper is needed.
    ///
    /// Empirically validated (macOS 26) that
    /// `Date.ISO8601FormatStyle().parse(_:)` accepts both
    /// `"2023-11-14T22:13:20Z"` (seconds precision) and
    /// `"2023-11-14T22:13:20.123Z"` (fractional-seconds),
    /// so the previous two-phase-fallback is no longer
    /// needed.  Matches our signers' output byte-for-byte.
