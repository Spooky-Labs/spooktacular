...
///    - `SPOOK_ADMIN_PRESENCE_BYPASS=1` — the opt-in flag.
///    - `SPOOK_ADMIN_PRESENCE_BYPASS_TOKEN=<token>` — a signed
...
public enum AdminPresenceGate {
    ...
    public static func requirePresence(
        ...
        if bypass {
            ...
            guard let rawToken = environment["SPOOK_ADMIN_PRESENCE_BYPASS_TOKEN"],
                  !rawToken.isEmpty else {
                log.error("Admin presence bypass refused: missing SPOOK_ADMIN_PRESENCE_BYPASS_TOKEN. reason=\(reason, privacy: .public)")
            ...
            // the SpookCore enum is out of scope for this agent, so
...
public enum AdminPresenceError: Error, LocalizedError {
    ...
    /// `SPOOK_ADMIN_PRESENCE_BYPASS=1` was set but the matching
    /// `SPOOK_ADMIN_PRESENCE_BYPASS_TOKEN` is missing. The
    ...
    /// No ``SpookCore/AuditSink`` was provided, so the bypass
    ...
    public var errorDescription: String? {
        ...
        case .bypassTokenMissing:
            "SPOOK_ADMIN_PRESENCE_BYPASS=1 was set but SPOOK_ADMIN_PRESENCE_BYPASS_TOKEN is missing — both are required."
        case .bypassVerifierNotConfigured:
    ...
    public var recoverySuggestion: String? {
        ...
        case .presenceUnavailable:
            "Configure a login password on the host. For genuinely headless environments (CI), set BOTH `SPOOK_ADMIN_PRESENCE_BYPASS=1` and `SPOOK_ADMIN_PRESENCE_BYPASS_TOKEN=<signed-consent-token>`. Every bypass is audited."
        case .bypassRefusedInStrictMode:
        ...
        case .bypassTokenMissing:
            "Mint a monthly consent token (`spook break-glass issue-presence-token --host <hostname>`) and export it as `SPOOK_ADMIN_PRESENCE_BYPASS_TOKEN`."
        case .bypassVerifierNotConfigured:
