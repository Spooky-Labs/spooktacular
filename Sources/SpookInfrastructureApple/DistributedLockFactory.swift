...
///
/// 1. **`SPOOK_DYNAMO_TABLE` set** → ``DynamoDBDistributedLock``.
///    Intended for cross-region Fortune-20 fleets where K8s Leases
///    can't bridge regions and file locks don't work at all.
///    `SPOOK_DYNAMO_REGION` (default `us-east-1`) and
///    `SPOOK_DYNAMO_ENDPOINT` (optional — LocalStack, custom VPC
...
///    `SPOOK_LOCK_DIR` (defaults to `~/.spooktacular/locks`).
///
...
public enum DistributedLockFactory {
    ...
    public static func makeFromEnvironment(
        ...
        if let table = environment["SPOOK_DYNAMO_TABLE"], !table.isEmpty {
            let region = environment["SPOOK_DYNAMO_REGION"] ?? "us-east-1"
            let endpoint = environment["SPOOK_DYNAMO_ENDPOINT"].flatMap(URL.init(string:))
        ...
        let lockDir = environment["SPOOK_LOCK_DIR"]
