...
extension Spooktacular {
    ...
    /// is not configured and no `SPOOKTACULAR_API_TOKEN` is set, use
    ...
    struct Serve: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            ...
            discussion: """
                ...
                When TLS is not configured and no SPOOKTACULAR_API_TOKEN is \
        ...
        var spookPath: String = ProcessInfo.processInfo.environment["SPOOKTACULAR_PATH"] ?? HTTPAPIServer.defaultSpookPath

        @Option(name: .customLong("tls-cert"),
                help: "Path to a PEM-encoded TLS certificate file. Falls back to SPOOKTACULAR_TLS_CERT_PATH.")
        var tlsCert: String?

        @Option(name: .customLong("tls-key"),
                help: "Path to a PEM-encoded TLS private key file. Falls back to SPOOKTACULAR_TLS_KEY_PATH.")
        var tlsKey: String?
        ...
        func run() async throws {
            ...
            let resolvedTLSCert = tlsCert ?? env["SPOOKTACULAR_TLS_CERT_PATH"]
            let resolvedTLSKey  = tlsKey  ?? env["SPOOKTACULAR_TLS_KEY_PATH"]

            // Validate flag combinations.
            let hasCert = resolvedTLSCert != nil
            let hasKey = resolvedTLSKey != nil
            if hasCert != hasKey {
                print(Style.error("Both --tls-cert and --tls-key (or SPOOKTACULAR_TLS_CERT_PATH / SPOOKTACULAR_TLS_KEY_PATH) must be provided together."))
                throw ExitCode.failure
            }
            if insecure && hasCert {
            ...
            // Tenancy mode
            let tenancyMode: TenancyMode = env["SPOOKTACULAR_TENANCY_MODE"] == "multi-tenant"
            ...
            // loss on restart.
            let roleStore = try JSONRoleStore(configPath: env["SPOOKTACULAR_RBAC_CONFIG"])
            ...
            // Walks SPOOKTACULAR_API_PUBLIC_KEYS_DIR for PEM files;
            ...
            if let keysDir = env["SPOOKTACULAR_API_PUBLIC_KEYS_DIR"] {
                ...
                if fm.fileExists(atPath: keysDir, isDirectory: &isDir), isDir.boolValue,
                   let names = try? fm.contentsOfDirectory(atPath: keysDir) {
                    ...
                    if !keys.isEmpty {
                        ...
                        print(Style.warning("SPOOKTACULAR_API_PUBLIC_KEYS_DIR at '\(keysDir)' has no valid PEMs"))
                    }
                } else {
                    print(Style.warning("SPOOKTACULAR_API_PUBLIC_KEYS_DIR at '\(keysDir)' is not a directory"))
                }
            }

            // Workload-identity OIDC issuer. When both
            // SPOOKTACULAR_OIDC_ISSUER_URL and an SEP / PEM signing key
            ...
            //
            // SPOOKTACULAR_HOST_IDENTITY_KEY_LABEL selects the SEP
            ...
            // agent's SPOOKTACULAR_HOST_PUBLIC_KEYS_DIR trust
            // directory.
            let hostKeyLabel = env["SPOOKTACULAR_HOST_IDENTITY_KEY_LABEL"] ?? "default"
            do {
                ...
                print(Style.dim("  Host public key (install into each agent's SPOOKTACULAR_HOST_PUBLIC_KEYS_DIR):"))
            ...
            // OpenTelemetry exporter. When SPOOKTACULAR_OTLP_ENDPOINT
            ...
            if let endpointStr = env["SPOOKTACULAR_OTLP_ENDPOINT"],
               let endpoint = URL(string: endpointStr) {
                let headers = env["SPOOKTACULAR_OTLP_HEADERS"]
                    .flatMap(parseOTLPHeaders) ?? [:]
                otelExporter = OTLPHTTPJSONExporter(config: .init(
                    endpoint: endpoint,
                    serviceName: env["SPOOKTACULAR_OTLP_SERVICE_NAME"] ?? "spooktacular",
            ...
            do {
                iamBindingStore = try JSONVMIAMBindingStore(
                    configPath: env["SPOOKTACULAR_IAM_BINDINGS_CONFIG"]
                )
            } catch {
