...
extension Spooktacular {
    ...
    /// emits the three `X-Spooktacular-*` headers for `curl` / any
    /// client to forward.
    ///
    ...
    struct SignRequest: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            ...
            abstract: "Sign an HTTP request and emit X-Spooktacular-* headers for curl / clients.",
            discussion: """
        ...
        func run() async throws {
            ...
            // Build canonical string + sign.  Apple's
            // modern, `Sendable` `Date.ISO8601FormatStyle`
            // default config emits `2023-11-14T22:13:20Z`
            // byte-identical to
            // `ISO8601DateFormatter[.withInternetDateTime]`,
            // so the wire format `SignedRequestVerifier`
            ...
            let triple: [(String, String)] = [
                ...
                ("X-Spooktacular-Signature", signatureB64)
            ]
