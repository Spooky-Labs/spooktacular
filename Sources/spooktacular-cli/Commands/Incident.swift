    ...
    struct Incident: AsyncParsableCommand {
        ...
        struct Status: AsyncParsableCommand {
            ...
            func run() async throws {
                ...
                let hostLabel = ProcessInfo.processInfo.environment["SPOOKTACULAR_HOST_IDENTITY_KEY_LABEL"] ?? "default"
                ...
                let dirs: [(String, String?)] = [
                    ("Host public keys (agents trust hosts)", env["SPOOKTACULAR_HOST_PUBLIC_KEYS_DIR"]),
                    ("Break-glass public keys", env["SPOOKTACULAR_BREAKGLASS_PUBLIC_KEYS_DIR"]),
                    ("API public keys", env["SPOOKTACULAR_API_PUBLIC_KEYS_DIR"]),
                ]
                ...
                Style.field("JSONL file", env["SPOOKTACULAR_AUDIT_FILE"] ?? "(unset)")
                Style.field("Immutable append", env["SPOOKTACULAR_AUDIT_IMMUTABLE_PATH"] ?? "(unset)")
                Style.field("Merkle signing", env["SPOOKTACULAR_AUDIT_MERKLE"] == "1" ? "enabled" : "disabled")
                Style.field("S3 Object Lock", env["SPOOK_AUDIT_S3_BUCKET"] ?? "(unset)")
                Style.field("SIEM webhook", env["SPOOKTACULAR_AUDIT_WEBHOOK_URL"] ?? "(unset)")

                // OIDC / IAM
                print("")
                Style.header("Workload identity")
                Style.field("OIDC issuer URL", env["SPOOKTACULAR_OIDC_ISSUER_URL"] ?? "(unset)")
                Style.field("OIDC issuer key",
                    env["SPOOKTACULAR_OIDC_ISSUER_KEY_LABEL"].map { "Keychain '\($0)' (SEP-bound)" }
                        ?? "(unset)"
                )
            }
        ...
        struct RotateHostIdentity: AsyncParsableCommand {
            ...
            func run() async throws {
                ...
                print("     SPOOKTACULAR_HOST_PUBLIC_KEYS_DIR. Agents now trust both keys.")
                print("  2. Update SPOOKTACULAR_HOST_IDENTITY_KEY_LABEL=\(newLabel) on every")
        ...
        struct AuditTail: AsyncParsableCommand {
            ...
            @Option(name: .customLong("audit-file"),
                    help: "Override the audit file path. Defaults to $SPOOKTACULAR_AUDIT_FILE.")
            var auditFile: String?

            func run() throws {
                let path = auditFile
                    ?? ProcessInfo.processInfo.environment["SPOOKTACULAR_AUDIT_FILE"]
                    ?? ""
                guard !path.isEmpty else {
                    print(Style.error("✗ No audit file configured. Set SPOOKTACULAR_AUDIT_FILE or pass --audit-file."))
                    throw ExitCode.failure
                }
                ...
                for line in lines.reversed() {
                    ...
                    // accepts both fractional and plain
                    // seconds variants — verified on
                    // macOS 26 — so the two-phase
                    // fallback collapses to one call.
                    let parsed = try? Date.ISO8601FormatStyle().parse(ts)
