...
extension Spooktacular {
    ...
    /// ✗ TLS not configured
    /// ⚠ SPOOKTACULAR_API_TOKEN not set
    ...
    struct Doctor: AsyncParsableCommand {
        ...
        static func check01TLSCertAndKey(env: [String: String]) -> CheckResult {
            let certPath = env["SPOOKTACULAR_TLS_CERT_PATH"]
            let keyPath = env["SPOOKTACULAR_TLS_KEY_PATH"]
            let fm = FileManager.default
            guard let certPath, let keyPath,
                  !certPath.isEmpty, !keyPath.isEmpty else {
                return fail(1, "TLS cert+key — SPOOKTACULAR_TLS_CERT_PATH and SPOOKTACULAR_TLS_KEY_PATH must both be set")
            }
            guard fm.isReadableFile(atPath: certPath) else {
                return fail(1, "TLS cert+key — SPOOKTACULAR_TLS_CERT_PATH=\(certPath) is not readable by this process")
            }
            guard fm.isReadableFile(atPath: keyPath) else {
                return fail(1, "TLS cert+key — SPOOKTACULAR_TLS_KEY_PATH=\(keyPath) is not readable by this process")
            }
        ...
        static func check02MutualTLS(env: [String: String]) -> CheckResult {
            let caPath = env["SPOOKTACULAR_TLS_CA_PATH"] ?? env["TLS_CA_PATH"]
            guard let caPath, !caPath.isEmpty else {
                return fail(2, "mTLS — SPOOKTACULAR_TLS_CA_PATH not set (client certs are NOT required)")
            }
            guard FileManager.default.isReadableFile(atPath: caPath) else {
                return fail(2, "mTLS — SPOOKTACULAR_TLS_CA_PATH=\(caPath) is not readable")
            }
        ...
        static func check04APITokenKeychain(env: [String: String]) -> CheckResult {
            ...
            if let tok = env["SPOOKTACULAR_API_TOKEN"], !tok.isEmpty {
                return warn(4, "API bearer token — Keychain empty, falling back to SPOOKTACULAR_API_TOKEN env var (not hardened; keychain status=\(status), \(note))")
            }
            return fail(4, "API bearer token — neither Keychain nor SPOOKTACULAR_API_TOKEN is set (keychain status=\(status), \(note))")
        }
        ...
        static func check05GuestAgentTokens(env: [String: String]) -> CheckResult {
            ...
            let names = [
                "SPOOKTACULAR_AGENT_TOKEN",
                "SPOOKTACULAR_AGENT_RUNNER_TOKEN",
                "SPOOKTACULAR_AGENT_READONLY_TOKEN"
            ]
            ...
            return warn(5, "Guest-agent tokens — no SPOOKTACULAR_AGENT_*_TOKEN env var and no Keychain entry at com.spooktacular.agent")
        }

        // MARK: - Item 06: RBAC active

        static func check06RBACActive(env: [String: String]) -> CheckResult {
            if let rbacPath = env["SPOOKTACULAR_RBAC_CONFIG"], !rbacPath.isEmpty {
                if FileManager.default.isReadableFile(atPath: rbacPath) {
                    return pass(6, "RBAC config readable: \(rbacPath)")
                }
                return fail(6, "RBAC — SPOOKTACULAR_RBAC_CONFIG=\(rbacPath) is not readable")
            }
            if let groupMap = env["SPOOKTACULAR_MACOS_GROUP_MAPPING"], !groupMap.isEmpty {
        ...
        static func check09AuditJSONL(env: [String: String]) -> CheckResult {
            guard let auditPath = env["SPOOKTACULAR_AUDIT_FILE"], !auditPath.isEmpty else {
                return fail(9, "Audit JSONL — SPOOKTACULAR_AUDIT_FILE not set")
            }
        ...
        static func check10AppendOnlyAudit(env: [String: String]) -> CheckResult {
            guard let path = env["SPOOKTACULAR_AUDIT_IMMUTABLE_PATH"], !path.isEmpty else {
                return fail(10, "Append-only audit — SPOOKTACULAR_AUDIT_IMMUTABLE_PATH not set")
            }
            let fm = FileManager.default
        ...
        static func check15InsecureOff(env: [String: String]) -> CheckResult {
            if env["SPOOKTACULAR_INSECURE_CONTROLLER"] == "1" {
                return fail(15, "Insecure — SPOOKTACULAR_INSECURE_CONTROLLER=1 (mTLS bypass active; do NOT ship)")
            }
            return pass(15, "Insecure — SPOOKTACULAR_INSECURE_CONTROLLER is not set")
        }
        ...
        static func check20IAMBindingStoreWritable(env: [String: String]) -> CheckResult {
            guard let path = env["SPOOKTACULAR_IAM_BINDINGS_CONFIG"], !path.isEmpty else {
                return warn(20, "IAM bindings — SPOOKTACULAR_IAM_BINDINGS_CONFIG unset (identity-token minting disabled)")
            }
            let fm = FileManager.default
            let dir = URL(filePath: path).deletingLastPathComponent().path
        ...
        static func check21AuditSinkCanWrite(env: [String: String]) -> CheckResult {
            guard let path = env["SPOOKTACULAR_AUDIT_FILE"], !path.isEmpty else {
                return warn(21, "Audit sink probe — SPOOKTACULAR_AUDIT_FILE unset, skipping")
            }
            let fm = FileManager.default
        ...
        static func check22SignedRequestKeys(env: [String: String]) -> CheckResult {
            guard let dir = env["SPOOKTACULAR_API_PUBLIC_KEYS_DIR"], !dir.isEmpty else {
                return warn(22, "Signed requests — SPOOKTACULAR_API_PUBLIC_KEYS_DIR unset (Bearer-token fallback only)")
            }
            let fm = FileManager.default
        ...
        /// Checks whether the SPOOKTACULAR_API_TOKEN environment variable is set.
        private func checkAPIToken() -> CheckResult {
            if let token = ProcessInfo.processInfo.environment["SPOOKTACULAR_API_TOKEN"],
               !token.isEmpty {
                return CheckResult(
                    status: .pass,
                    message: "SPOOKTACULAR_API_TOKEN set"
                )
            } else {
                return CheckResult(
                    status: .warning,
                    message: "SPOOKTACULAR_API_TOKEN not set"
                )
            }
        }
