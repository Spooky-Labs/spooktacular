...
// `spook remote install-agent` no longer exists. Guest-side
// functionality is delivered by the in-guest
// `Spooktacular Guest Tools.app`, installed via
// `DiskInjector.installGuestTools` (driven by the
// `--guest-tools` flag on `spook create` or the GUI
// "Install Guest Tools" button).


// MARK: - Shared Helpers

extension Spooktacular.Remote {
...
extension Spooktacular.Remote {
    ...
    struct Exec: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            let agent = try Spooktacular.Remote.requireAgent(for: name)
...
extension Spooktacular.Remote.Clipboard {
...
extension Spooktacular.Remote.Apps {
