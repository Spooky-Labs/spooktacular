...
/// - `SPOOKTACULAR_BUNDLE_PROTECTION=none` forces `.none` even on
///   laptops (dev-loop convenience; operators accept the risk).
/// - `SPOOKTACULAR_BUNDLE_PROTECTION=cufua` forces CUFUA even on
...
public enum BundleProtection {
    ...
    public enum Policy: Sendable, Equatable {
        /// Environment override: operator set `SPOOKTACULAR_BUNDLE_PROTECTION=none`.
        case overrideNone
        /// Environment override: operator set `SPOOKTACULAR_BUNDLE_PROTECTION=cufua`.
        case overrideCUFUA
    ...
    /// priority order:
    ///
    /// 1. `SPOOKTACULAR_BUNDLE_PROTECTION` env var — operator intent
    ...
    public static func recommendedPolicy(
        ...
        // Tier 1: env var wins.
        switch environment["SPOOKTACULAR_BUNDLE_PROTECTION"]?.lowercased() {
        case "none":
