...
public enum SSHExecutor {
    ...
    /// `SPOOKTACULAR_SSH_HOST_KEY_TRUST=strict` /
    /// `SPOOKTACULAR_SSH_KNOWN_HOSTS=...` so operators don't have to
    /// edit source to enforce it.
    public static var defaultHostKeyTrust: HostKeyTrust {
        let env = ProcessInfo.processInfo.environment
        let mode = env["SPOOKTACULAR_SSH_HOST_KEY_TRUST"]
        let path = env["SPOOKTACULAR_SSH_KNOWN_HOSTS"]
