...
/// public keys via `SPOOKTACULAR_API_PUBLIC_KEYS_DIR`; every request
/// except `GET /health` must carry `X-Spooktacular-Timestamp`,
/// `X-Spooktacular-Nonce`, and `X-Spooktacular-Signature` headers.
///
/// ## Endpoints
///
...
public actor HTTPAPIServer {
    ...
    /// except `GET /health` must present the three `X-Spooktacular-*`
    /// headers (timestamp, nonce, signature) and the signature
    /// must verify against the operator-provisioned trust
    /// allowlist.
    ///
    /// The static Bearer-token path (`SPOOKTACULAR_API_TOKEN`) was
    ...
    /// Maximum concurrent connections. Override with `SPOOKTACULAR_MAX_CONNECTIONS` env var.
    private let maxConcurrentConnections: Int

    /// Current number of active connections.
    private var activeConnectionCount: Int = 0

    /// Per-client rate limit: max requests per minute. Override with `SPOOKTACULAR_RATE_LIMIT` env var.
    ...
    /// `SPOOKTACULAR_MAX_REQUEST_BYTES`.
    ...
    /// Defaults to 30 s; override with `SPOOKTACULAR_REQUEST_TIMEOUT_SECONDS`.
    ...
    /// with `SPOOKTACULAR_HEADER_TIMEOUT_SECONDS`.
    ...
    /// Override with `SPOOKTACULAR_READ_RATE_LIMIT`.
    ...
    /// Override with `SPOOKTACULAR_WRITE_RATE_LIMIT`.
    ...
    /// `SPOOKTACULAR_TRUST_FORWARDED_FOR=1`.
    ...
    public init(
        ...
        self.maxConcurrentConnections = Int(ProcessInfo.processInfo.environment["SPOOKTACULAR_MAX_CONNECTIONS"] ?? "") ?? 50
        let overallLimit = Int(ProcessInfo.processInfo.environment["SPOOKTACULAR_RATE_LIMIT"] ?? "") ?? 120
        self.maxRequestsPerMinute = overallLimit
        self.readRateLimit = Int(ProcessInfo.processInfo.environment["SPOOKTACULAR_READ_RATE_LIMIT"] ?? "") ?? overallLimit
        self.writeRateLimit = Int(ProcessInfo.processInfo.environment["SPOOKTACULAR_WRITE_RATE_LIMIT"] ?? "")
            ?? max(1, (overallLimit + 2) / 3)
        self.trustForwardedFor = (ProcessInfo.processInfo.environment["SPOOKTACULAR_TRUST_FORWARDED_FOR"] ?? "0") == "1"
        self.maxRequestBytes = Int(ProcessInfo.processInfo.environment["SPOOKTACULAR_MAX_REQUEST_BYTES"] ?? "")
            ?? Self.defaultMaxRequestBytes
        self.requestTimeoutSeconds = Double(ProcessInfo.processInfo.environment["SPOOKTACULAR_REQUEST_TIMEOUT_SECONDS"] ?? "")
            ?? Self.defaultRequestTimeoutSeconds
        self.headerTimeoutSeconds = Double(ProcessInfo.processInfo.environment["SPOOKTACULAR_HEADER_TIMEOUT_SECONDS"] ?? "")
    ...
    // VM name validation is centralized in
    // `SpooktacularPaths.isValidVMName(_:)` — a pure,
    // `Sendable`-by-construction predicate over ASCII
    // bytes.  Using that instead of a local `Regex<Substring>`
    // static eliminates the `nonisolated(unsafe)` wrapper the
    // Regex engine would otherwise require (Regex isn't
    // Sendable per Swift 6).
    ...
    private func handleHealth() -> HTTPResponse {
        HTTPResponse.ok(HealthResponse(service: "spooktacular", version: "1.0.1"))
    }
    ...
    /// (version 0.0.4). Requires authentication when `SPOOKTACULAR_API_TOKEN`
        ...
        guard FileManager.default.isExecutableFile(atPath: spookPath) else {
            ...
            return HTTPResponse.error(
                message: "spook binary not found at '\(spookPath)'. "
                    + "Set --spook-path or the SPOOKTACULAR_PATH environment variable.",
                statusCode: 500
            )
        }
...
public enum HTTPAPIServerError: Error, Sendable, LocalizedError, Equatable {
    ...
    public var recoverySuggestion: String? {
        ...
        case .missingSignatureVerifier:
            "Point SPOOKTACULAR_API_PUBLIC_KEYS_DIR at a directory of operator/controller PEM public keys. Production requires both TLS and signature verification — use --insecure to bypass (not recommended)."
        case .tlsRequired:
