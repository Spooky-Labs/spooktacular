...
///
/// ```bash
/// SPOOKTACULAR_AUDIT_IMMUTABLE=1
...
public enum AppendOnlyError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .kernelFlagFailed(let p):
            "Could not set UF_APPEND on \(p). The filesystem or ACLs may not support BSD file flags — choose a different audit path or disable SPOOKTACULAR_AUDIT_IMMUTABLE."
        }
    }
    ...
    public var recoverySuggestion: String? {
        ...
        case .kernelFlagFailed:
            "Move SPOOKTACULAR_AUDIT_IMMUTABLE_PATH to an APFS volume — UF_APPEND requires BSD file flags. SMB/NFS/external-disk paths often silently reject `chflags`."
        }
    }
}
