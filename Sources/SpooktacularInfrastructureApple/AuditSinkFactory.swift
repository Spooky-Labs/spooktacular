...
public enum AuditSinkFactory {
    ...
    /// Requires `SPOOKTACULAR_AUDIT_SIGNING_KEY_LABEL` (the SEP Keychain
    /// label). The legacy software-key path via
    /// `SPOOKTACULAR_AUDIT_SIGNING_KEY_PATH` has been removed — Apple's
...
public enum AuditSinkFactoryError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .merkleKeyLabelRequired:
            return "Merkle audit is enabled but SPOOKTACULAR_AUDIT_SIGNING_KEY_LABEL is unset or empty. Set it to a non-empty Keychain label — the SEP-backed key will be generated on first use and reused thereafter. The legacy SPOOKTACULAR_AUDIT_SIGNING_KEY_PATH software-key path has been removed."
        }
    }
}
