...
/// `X-Spooktacular-Audit-Signature` header — `hex(hmac-sha256(key, body))`
...
public actor WebhookAuditSink: AuditSink {
    ...
    private func post(body: Data) async throws {
        ...
        if let key = config.hmacKey {
            ...
            req.setValue(hex, forHTTPHeaderField: "X-Spooktacular-Audit-Signature")
        }
        req.httpBody = body
    ...
    // `JSONEncoder` is `Sendable` since macOS 13; a `let`
    // on a Sendable type is safe to share across actors
    // without the `nonisolated(unsafe)` escape hatch.
