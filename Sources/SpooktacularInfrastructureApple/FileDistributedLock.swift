...
/// ## Configuration
///
/// Set `SPOOKTACULAR_LOCK_DIR` to a shared directory accessible by all hosts
...
public actor FileDistributedLock: DistributedLockService {
    ...
    public init(lockDir: String? = nil) {
        self.lockDir = lockDir
            ?? ProcessInfo.processInfo.environment["SPOOKTACULAR_LOCK_DIR"]
...
public enum FileLockError: Error, LocalizedError, Sendable, Equatable {
    ...
    public var recoverySuggestion: String? {
        ...
        case .openFailed(let path, let code):
            code == EACCES
                ? "The process doesn't have write access to '\(path)'. Check SPOOKTACULAR_LOCK_DIR ownership and mode."
